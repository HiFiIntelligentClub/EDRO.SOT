...
/*function сДоСимвола($_сВход, $_сДо='?') // Слово  SAME FUNCTIONS DEPRICATED
	{
	$сСлово		='';
	if(empty($_сВход))
	    {
...
function сОтСимвола($_сВход, $_сОт='?') // Слово  SAME FUNCTIONS 2 DEPRICATED
	{
	$сСлово		='';
	$фСимволНайден	=false;
...
function сНачОтСимвола($_сВход, $_сОт='?') // Слово  SAME FUNCTIONS 2
	{
...
function сНачДоСимвола($_сВход, $_сДо='?') // Слово  SAME FUNCTIONS
	{
	if(empty($_сВход)){return '';}
	$сВход		=(string)$_сВход;
	$сСлово		='';
...
function сНачОтДоСимвола($_сВход, $_сОт, $_сДо, $_nu1BeginOffset=1)
	{
	if(empty($_сВход)){return '';}
	$сВход			=(string)$_сВход;
	$сОтДо			='';
	...
	$сКонецСтр		=сНачОтСимвола($сВход, $сОт);

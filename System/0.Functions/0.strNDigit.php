...
function сНачОтДоСимвола($_сСтр, $_сОт, $_сДо, $_nu1BeginOffset=1)
	...
	return сНачДоСимвола(substr(сНачОтСимвола($сСтр, $сОт), $nu1BeginOffset), $сДо);
	}
...
function мУрлРазобратьПоток($_сСтр)
	{
	//echo $_сСтр."\n";
	echo  $м['strAddress']	= сНачОтДоСимвола($_сСтр, '/', ':', 2);
	echo "\n";
	...
	echo $м['intPort'];
	echo "\n";
	echo $м['strGet']		= сКонцДоСимвола($_сСтр, '/');
	echo "\n";
	return $м;
	}
